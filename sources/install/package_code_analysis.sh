...
function install_trufflehog() {
    ...
    trufflehog_url=$(curl --location --silent "https://api.github.com/repos/trufflesecurity/trufflehog/releases/latest" | grep 'browser_download_url.*linux_'"$arch"'.tar.gz"' | grep -o 'https://[^"]*')
    if [[ -z "$trufflehog_url" ]]; then
        criticalecho-noexit "Failed to fetch trufflehog download URL from GitHub API!" && return 1
    fi
